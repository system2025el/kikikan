...
export const selectIdoDen = async (sagyoKbnId: number, sagyoSijiId: number, sagyoDenDat: string, sagyoId: number) => {
  ...
  try {
    return await supabase
      ...
      .from('v_ido_den3_lst')
      .select(
        'ido_flg, juchu_flg, juchu_meisai, kizai_id, kizai_nam, kizai_shozoku_id, rfid_yard_qty, rfid_kics_qty, plan_juchu_qty, plan_low_qty, plan_qty, result_qty, result_adj_qty, diff_qty, ctn_flg'
      )
