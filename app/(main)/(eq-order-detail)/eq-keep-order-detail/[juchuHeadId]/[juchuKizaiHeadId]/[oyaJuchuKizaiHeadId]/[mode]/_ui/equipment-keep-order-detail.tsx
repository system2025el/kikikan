...
export const EquipmentKeepOrderDetail = (props: {
  ...
  const handleEqAllSelect = useStableCallback(() => {
    ...
    if (selectEq.length === keepJuchuKizaiMeisaiList.filter((data) => !data.delFlag).length) {
      setKeepJuchuKizaiMeisaiList((prev) => prev.map((data) => ({ ...data, selected: false })));
      return;
    } else {
  ...
  const handleCtnAllSelect = useStableCallback(() => {
    ...
    if (selectCtn.length === keepJuchuContainerMeisaiList.filter((data) => !data.delFlag).length) {
      setKeepJuchuContainerMeisaiList((prev) => prev.map((data) => ({ ...data, selected: false })));
      return;
    } else {
   ...
   */
  const handleKicsShukoChange = async (newDate: Date | null) => {
    ...
    <>
                ...
                <Grid2 container p={2} spacing={2}>
                  <Grid2 order={{ xl: 1 }} width={300}>
                    ...
                    <Grid2>
                      ...
                      <DateTime
                        value={oyaJuchuKizaiHeadData.kicsShukoDat && new Date(oyaJuchuKizaiHeadData.kicsShukoDat)}
                        disabled
                      />
                    </Grid2>
                    <Grid2>
                      ...
                      <DateTime
                        value={oyaJuchuKizaiHeadData.yardShukoDat && new Date(oyaJuchuKizaiHeadData.yardShukoDat)}
                        disabled
                      />
                    </Grid2>
                  </Grid2>
                  <Grid2 width={300} order={{ xl: 4 }}>
                    ...
                    <Grid2>
                      <TextField defaultValue={'K'} disabled sx={{ width: '10%', minWidth: 50 }} />
                      <DateTime
                        value={oyaJuchuKizaiHeadData.kicsNyukoDat && new Date(oyaJuchuKizaiHeadData.kicsNyukoDat)}
                        disabled
                      />
                    </Grid2>
                    <Grid2>
                      <TextField defaultValue={'Y'} disabled sx={{ width: '10%', minWidth: 50 }} />
                      <DateTime
                        value={oyaJuchuKizaiHeadData.yardNyukoDat && new Date(oyaJuchuKizaiHeadData.yardNyukoDat)}
                        disabled
                      />
                    </Grid2>
                  </Grid2>
                ...
                <>
                  <Box width="100%">
                    <Grid2 container my={1} mx={1} spacing={2}>
                      ...
                      {(keepJuchuKizaiMeisaiList.filter((d) => !d.delFlag).length !== 0 ||
                        keepJuchuContainerMeisaiList.filter((d) => !d.delFlag).length !== 0) && (
                        <Button
                          color="error"
                          disabled={
                            !edit ||
      ...
      <DeleteAlertDialog
        selectedLength={
          ...
          keepJuchuContainerMeisaiList.filter((data) => !data.delFlag && data.selected).length
        }
        open={deleteOpen}
