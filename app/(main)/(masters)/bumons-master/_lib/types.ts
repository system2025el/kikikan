...
export const BumonsMasterDialogSchema = z.object({
  ...
  mem: z.string().max(200, '200文字以内で入力してください').optional().nullable(),
  daibumonId: z.number({ message: '数字を入力してください' }).optional().nullable(),
  shukeibumonId: z.number({ message: '数字を入力してください' }).optional().nullable(),
});
