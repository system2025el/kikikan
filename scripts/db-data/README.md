...
| `t_juchu_kizai_honbanbi`（`_template.sql`） | 本番日の受注ヘッダー単位テンプレートを `juchu_kizai_head_id = 0` で作成（1,692行 / 499受注） | 2026-08-31 | 未適用 |
...
`t_juchu_kizai_honbanbi_template.sql` は本番日（種別10/20/30/40）の入力を明細画面から伝票画面へ移す変更に対応するものです。**アプリ側のデプロイと同時に適用してください**（テンプレートが無いと伝票画面で本番日が空に見えます）。適用前に `pg_dump` でこのテーブルのバックアップを取ること。テンプレート行へのアクセスは `app/_lib/db/tables/t-juchu-honbanbi.ts` に閉じてあり、将来テンプレート専用テーブルへ移す場合もこのファイルの中だけで完結します。
