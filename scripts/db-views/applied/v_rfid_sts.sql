-- 適用状況: 開発環境(preview/public) 2026-08-25 / 本番 2026-09-03
-- v_rfid_sts 高速化版（第2弾）
--
-- 前提: v_rfid_sts に対する2回目の高速化です。1回目は「5つの相関サブクエリ →
--       ウィンドウ関数3つに統合」で、本番には 2026-08-19 に適用済みでした。
--       同名ファイルは1ビュー1ファイル（＝本番の現在の定義）の原則で本ファイルに
--       置き換わっているため、1回目の定義とそのロールバックSQLが必要な場合は
--       git履歴から取得してください。
--       本ファイルに対応する v_rfid_sts.rollback.sql は、1回目のウィンドウ関数版
--       （＝本ファイル適用前の本番の定義）に戻すものです。
--
